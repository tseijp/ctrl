...
export class SubscriptionModel {
  ...
  // サブスクリプション情報を更新
  static async update(id: string, data: Partial<SubscriptionCreateInput>, env?: Env): Promise<Subscription | null> {
    const db = getDb(env);
  ...
  static async syncFromStripeEvent(event: any, env?: Env): Promise<Subscription | null> {
    ...
    // 既存のサブスクリプションを検索
    const existingSubscription = await SubscriptionModel.findByStripeSubscriptionId(stripeSubscription.id, env);
    
    if (existingSubscription) {
      // 既存のサブスクリプションを更新
      return await SubscriptionModel.update(existingSubscription.id, {
      ...
      // 顧客IDで検索
      const customerSubscription = await SubscriptionModel.findByStripeCustomerId(stripeSubscription.customer, env);
      
      if (customerSubscription) {
        // 既存の顧客サブスクリプションを更新
        return await SubscriptionModel.update(customerSubscription.id, {
