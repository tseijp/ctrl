...
export class ThreadModel {
  ...
  static async create(data: ThreadCreateInput, env?: Env): Promise<Thread> {
    const db = getDb(env);
    const result = await db.insert(threads)
      .values(data)
      .returning();
  ...
  // スレッド情報を更新
  static async update(id: string, data: Partial<Omit<ThreadCreateInput, 'projectId'>>, env?: Env): Promise<Thread | null> {
    const db = getDb(env);
  ...
  // スレッドの位置を更新
  static async updatePosition(id: string, positionX: string, positionY: string, env?: Env): Promise<Thread | null> {
    const db = getDb(env);
    const result = await db.update(threads)
      .set({ 
        positionX,
