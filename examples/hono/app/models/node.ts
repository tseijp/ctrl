...
export class NodeModel {
        ...
        static async findByProjectId(projectId: string, env?: Env): Promise<Node[]> {
                const db = getDb(env)
                const result = await db.select().from(nodes).where(eq(nodes.projectId, projectId))
                return result
        }
        ...
        static async findByLayerId(layerId: string, env?: Env): Promise<Node[]> {
                const db = getDb(env)
                const result = await db.select().from(nodes).where(eq(nodes.layerId, layerId))
                return result
        }
        ...
        static async findByProjectIdAndLayerId(projectId: string, layerId: string, env?: Env): Promise<Node[]> {
                ...
                const result = await db
                        .select()
                        .from(nodes)
                        .where(and(eq(nodes.projectId, projectId), eq(nodes.layerId, layerId)))
                return result
        }
        ...
        static async update(
                id: string,
                data: Partial<Omit<NodeCreateInput, 'projectId' | 'layerId'>>,
                env?: Env
        ): Promise<Node | null> {
                const db = getDb(env)
                const result = await db
