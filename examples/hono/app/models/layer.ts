...
export class LayerModel {
  ...
  static async create(data: LayerCreateInput, env?: Env): Promise<Layer> {
    ...
    const maxOrderResult = await db.select({ maxOrder: layers.orderIndex })
      .from(layers)
    ...
    // レイヤーを作成
    const result = await db.insert(layers)
  ...
  // レイヤー情報を更新
  static async update(id: string, data: Partial<Omit<LayerCreateInput, 'projectId'>>, env?: Env): Promise<Layer | null> {
    const db = getDb(env);
    const result = await db.update(layers)
