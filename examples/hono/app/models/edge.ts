...
export class EdgeModel {
  ...
  static async findBySourceNodeId(sourceNodeId: string, env?: Env): Promise<Edge[]> {
    ...
    const result = await db.select()
      .from(edges)
      .where(eq(edges.sourceNodeId, sourceNodeId));
    return result;
  }
  ...
  static async findByTargetNodeId(targetNodeId: string, env?: Env): Promise<Edge[]> {
    ...
    const result = await db.select()
      .from(edges)
      .where(eq(edges.targetNodeId, targetNodeId));
    return result;
  }
  ...
  static async findByNodeId(nodeId: string, env?: Env): Promise<Edge[]> {
    const db = getDb(env);
    const sourceEdges = await db.select()
      .from(edges)
      .where(eq(edges.sourceNodeId, nodeId));
    
    const targetEdges = await db.select()
      .from(edges)
  ...
  static async create(data: EdgeCreateInput, env?: Env): Promise<Edge> {
    const db = getDb(env);
    const result = await db.insert(edges)
      .values(data)
      .returning();
  ...
  // エッジ情報を更新
  static async update(id: string, data: Partial<Omit<EdgeCreateInput, 'projectId' | 'sourceNodeId' | 'targetNodeId'>>, env?: Env): Promise<Edge | null> {
    const db = getDb(env);
    const result = await db.update(edges)
  ...
  static async deleteByNodeId(nodeId: string, env?: Env): Promise<number> {
    ...
    const sourceResult = await db.delete(edges)
      .where(eq(edges.sourceNodeId, nodeId))
      .returning();
    
    const targetResult = await db.delete(edges)
      .where(eq(edges.targetNodeId, nodeId))
      .returning();
